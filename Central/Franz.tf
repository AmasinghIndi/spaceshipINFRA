...
# Create a resource group
resource "azurerm_resource_group" "franz-rg-US" {
  name     = "franz-rg-US"
  location = "East US"
  tags = {
    owner = "Franz.Martinek@redbull.com"
...
module "franz-vm-us" {
  source                        = "./modules/vm"
  nic_name                      = "franz-nic"
  location                      = azurerm_resource_group.franz-rg-US.location
  resource_group_name           = azurerm_resource_group.franz-rg-US.name
  //subnet_id                   = azurerm_subnet.franz-subnet.id
  # when using vnet module:
  subnet_id                     = module.franz-vnet-us.subnet_id
  vm_name                       = "franzVM"
  vm_size                       = "Standard_DS1_v2"
  os_disk_name                  = "example-os-disk"
  image_publisher               = "Canonical"
